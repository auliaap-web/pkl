<?php

namespace App\Filament\Resources;

use App\Filament\Resources\ProgresResource\Pages;
use App\Filament\Resources\ProgresResource\RelationManagers;
use App\Models\Progres;
use Filament\Forms;
use Filament\Forms\Form;
use Filament\Resources\Resource;
use Filament\Tables;
use Filament\Tables\Table;
use Illuminate\Database\Eloquent\Builder;
use Illuminate\Database\Eloquent\SoftDeletingScope;

class ProgresResource extends Resource
{
    protected static ?string $model = Progres::class;

    protected static ?string $navigationIcon = 'heroicon-o-rectangle-stack';

    public static function form(Form $form): Form
    {
        return $form
            ->schema([
                Forms\Components\TextInput::make('index1'),
                Forms\Components\TextInput::make('index2'),
                Forms\Components\TextInput::make('index3'),
                Forms\Components\TextInput::make('index3'),
                Forms\Components\TextInput::make('index4'),
                Forms\Components\TextInput::make('index5'),
            ]);
    }

    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                //
            ])
            ->filters([
                //
            ])
            ->actions([
                Tables\Actions\EditAction::make(),
            ])
            ->bulkActions([
                Tables\Actions\BulkActionGroup::make([
                    Tables\Actions\DeleteBulkAction::make(),
                ]),
            ]);
    }

    public static function getRelations(): array
    {
        return [
            //
        ];
    }

    public static function getPages(): array
    {
        return [
            'index' => Pages\ListProgres::route('/'),
            'create' => Pages\CreateProgres::route('/create'),
            'edit' => Pages\EditProgres::route('/{record}/edit'),
        ];
    }
}
